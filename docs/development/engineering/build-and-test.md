...
> verified `build/ai-namecard-full.bin` at offset `0x0` only when the
...
Never upload the app-only `build/ai-namecard.bin` to the community. Only the
validated `build/ai-namecard-full.bin` contains the structure the
