...
> `build/ai-namecard-full.bin` 写入空白设备；对已有身份的设备，只有合并文件
...
固件门禁使用全新的临时构建目录，并从仓库 `sdkconfig.defaults` 生成隔离的 `sdkconfig`。它不会读取或覆盖开发者根目录的 `sdkconfig`，只把验证通过的合并镜像复制到 `build/ai-namecard-full.bin`。门禁同时强制检查[小程序 BLE 兼容契约](ble-recovery-compatibility.zh_CN.md)：保护分区地址、应用大小、分区表 MD5、保护区数据不入包，以及 Recovery bootloader hook。
...
社区只能上传验证通过的 `build/ai-namecard-full.bin`，不得上传应用单镜像
`build/ai-namecard.bin`，后者没有小程序可安全解析与转换的完整结构。
