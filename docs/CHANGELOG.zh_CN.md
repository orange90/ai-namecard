...
- FoloCard 0.5.1 补充 BLE 连接失败提示：重新配对前，先在系统蓝牙设置中忽略已有的 FoloCard。
...
- 修正 Codex 周活跃摘要为“本周活跃：N天”，移除设备字体不支持的中间点字符。

- FoloCard 0.5.0 将确认后的 BLE 同步移入已安装的本机组件，移除扩展的回环网络请求和启动码输入；macOS 安装器会把 Python 运行环境复制到隐私保护的项目目录之外，确保 Brave 能可靠启动组件。Codex 摘要改为单行本周活跃天数，删除易误解的 `84 DAYS`，无历史数据时提示同步，并通过串口记录当前显示摘要；资料文本会先规范化到固件传输协议，事务式名片保存会在写入前回收非当前 NVS 槽，接近 8 KiB 的更新不再需要同时容纳三份名片；同时记录被动 NTAG213 无法按屏幕页面动态切换 NFC 的限制。

- 移除 FoloCard“编辑与确认”区域，包括手动资料填写、JSON 编辑和演示数据入口；采集结果直接用于预览及确认后的设备同步。

- 修复知乎和小红书头像采集：过滤懒加载占位图、依次尝试可信 CDN 候选地址，并按清单声明的通配主机权限检查；设备预览现在会显示名片中实际保存的 RGB565 头像。

- FoloCard 0.4.0 新增一次性本机组件安装器，由浏览器自动唤起每日 Token 读取，无需启动码；修复 Codex 迁移后的 cloud analytics 地址与纯文本额度重置时间解析，仍拒绝任意页面跳转。

- FoloCard 0.3.0 通过自带本机桥接扫描 Codex 每日 Token，不依赖 CodexBar 或其命令行；每日明细与热力图独立于浏览器额度，失败保留旧数据，分叉或交错记录无法完整统计时明确标注。只读取 Token 时无需 BLE 设备。

- FoloCard 0.2.2 将站点、头像 CDN 和本机桥接访问设为安装时声明的权限，移除读取与同步时的追加权限申请。

- 参考 CodexBar 接入基于浏览器会话的 Codex 用量接口，分别预览主/次周期额度和 Credits 余额，并支持页面兜底、登录确认及请求限流提示。修正重置时间为设备协议要求的秒级格式。

- FoloCard 改为一键自动查找浏览器中已登录的 Codex、知乎、小红书账号，分站点展示进度及登录/验证提示；后台采集独立保存成功结果并更新设备预览，不会自动同步到设备。

- FoloCard 插件新增可交互的 240 × 320 FoloToy 屏幕效果预览，复现 Codex 活动/用量和知乎/小红书名片布局，并支持切换页面及模拟 OK 查看详情。

- 修复 FoloCard 对知乎子页面和成就文案、小红书嵌套计数、Codex 剩余额度及相对重置时间的采集。插件独立页保留来源标签页，采集自动保存到本机，头像失败不再丢弃资料更新。

- 新增带 Codex、知乎和小红书品牌化数据面板的 FoloCard 离线名片、主动开启的加密 BLE 同步、本机头像转换、可编辑浏览器扩展及 macOS 本机桥接工具。
